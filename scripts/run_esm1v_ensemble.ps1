...
$ErrorActionPreference = "Stop"`r`n$PSNativeCommandUseErrorActionPreference = $true
...
if ($ResolvedDevice -eq "cpu" -and -not $AllowCpu) {
    throw "CUDA was not detected by $Python. ESM-1v is large; rerun with -AllowCpu only if you intentionally want a long CPU job."
}
...
for ($i = 0; $i -lt $models.Length; $i++) {
    ...
    Write-Host "Scoring $model on $ResolvedDevice with $Python..."
